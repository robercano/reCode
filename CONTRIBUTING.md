...
pipeline it ships to consumers, using a self-hosted adapter (`self/gates.json`) instead of the
...
- **`module:<name>`** — routing, not approval. It maps an issue to one of this repo's self-hosted modules
  (`docs`, `harness` → `.claude`, `examples`, `ci` → `.github` — see `self/gates.json`) and therefore
...
3. **Gate** — before anything is called done it must pass this repo's self-hosted gates
   (`GATES_FILE=self/gates.json bash .claude/scripts/gate.sh <build|lint|test>`), enforced by a `Stop`
   hook, not by asking the model nicely.
4. **Review** — gated changes go to reviewer lenses (`correctness`, `tests` per `self/gates.json` →
...
## What NOT to touch
- **`self/`** — this repo's own self-hosting adapter and loop config (module map, gate commands,
  ...
  reCode's own configuration (see `self/README.md`). Treat it as owner-maintained; if a change to the

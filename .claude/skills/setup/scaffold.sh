...
# pointed at `${CLAUDE_PLUGIN_ROOT:-$CLAUDE_PROJECT_DIR/.claude}/scripts/...` (issue #134 —
# this repo no longer vendors `agents/commands/hooks/scripts/skills` into consumer repos, so
# the plugin must stay ENABLED for both its own `hooks/hooks.json` and this file's hooks to
# resolve; the `${CLAUDE_PLUGIN_ROOT:-...}` fallback only degrades to the local `.claude/`
# copy for the self-hosting case, where there is no distinct plugin install). If the consumer
# already has a settings.json (likely — it may carry their own enabledPlugins/
# extraKnownMarketplaces while installing/updating), scaffold.sh leaves it completely
# untouched; SKILL.md instructs merging the runtime hooks in by hand in that case.

...
> **Documentation reference (not auto-discovered).** This file lives in `.claude/self/` and documents the
> self-hosting loop. The registered slash command `/pr-loop-self` is `.claude/commands/pr-loop-self.md`,
> which is excluded from plugin distribution to downstream projects via `.claude/.claude-plugin/.gitignore`
> (see `.claude/self/README.md` and `.claude/.claude-plugin/README.md`). For arming the loop in an interactive
> session, ask Claude to use `/pr-loop-self` directly, or read `.claude/self/pr-loop-self.md` and follow it
> (the cron tick prompt it arms is fully self-contained).

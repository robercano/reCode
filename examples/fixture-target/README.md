...
(`self/smoke-fanout.sh`, issue #64 — Phase 2 of #11). It is **not** a worked example to copy
...
2. Plays a *recorded implementer*: isolated worktree → applies a canned diff
   (`self/smoke/implementer.patch`) → asserts the diff stays inside the `fixture-core`
   module boundary (`src/`).
